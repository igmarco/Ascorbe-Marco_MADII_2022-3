...
Primero, importaremos nuestros datos de nuevo, de modo que nos sirva este apartado de forma independiente al resto, como si partiéramos de cero.
...
Recordemos que teníamos valores nulos en nuestros datos, de modo que hemos de volver a imputarlos.
...
Primero, vamos a hacer el Análisis de Componentes Principales de manera "artesanal", así como vimos en la práctica de la asignatura, por mayor completitud. Para ello, calculamos la matriz de covarianzas.
...
Calculamos los valores y vectores propios de la matriz de covarianzas.
...
Vemos que se corresponde con la suma de las varianzas de nuestras variables.

```{r}
100*cumsum(S_vvprop$values)/var_tot
```

Con tan solo las dos primeras componentes llegamos al 99% de varianza acumulada. Aun así, vamos a aplicar el método del codo para comprobarlo.
...
Es evidente que son dos componentes. Aunque con una podríamos obtener buenos resultados, es preferible tener un par de componentes para ayudarnos en la representación.
...
En la primera figura no llegamos a ver nada realmente útil, pero sí que encontramos algunos géneros distanciados del resto. Serán obras muy populares, con muchas reseñas y valoraciones. El resto está aglomerado en un cono y es difícil ver nada. Esto sucede tanto para la primera componente como para la segunda.
...
De nuevo, parece conveniente quedarse con k = 2.
...
Vemos qué características influyen más en cada componente. En general, las variables del número de valoraciones parecen tener mayor peso que las demás. Recuperamos el dataset original y le añadimos las dos componentes.
...
Vemos que los gráficos generados son prácticamente idénticos a los obtenidos por el método artesanal. En el primero concretamente parece ser posible apreciar una división en grupos por medio del eje de la primera componente.
...
Aquí parece ser más complicado decidirse, y podría paracer que lo mejor es k=5. Para poder visualizar los resultados de forma sencilla, consideraremos k = 2.
...
Realmente no se puede sacar demasiado en claro de esta gráfica, se nota que nuestras variables numéricas no pueden ser negativas y son cortadas en ese eje. No obstante, se pueden apreciar las tendencias de los géneros: pocos puntos veres o azules (historicos y romance en ese orden) se llegan a distanciar del conglomerado, mientras que hay aglomeraciones estiradas paralelas de géneros destacados, como fantasía o ficción.

Figura con mayor detalle en $Figura10.7$.

## 3. Análisis Factorial (AF)

Volvemos a ejecutar la importación de paquetes y de los datos.
...
Tratamos de nuevo los valores nulos.
...
Volvemos a realizar la selección de variables numéricas.
...
Vamos a comenzar realizando un Análisis Factorial "Artesanal". Conseguimos el número de variables y la matriz de correlaciones de nuestros datos.
...
Hallamos los porcentajes acumulados de varianza.

```{r}
cumvar <- 100*cumsum(Rprop$values)/p
```

Utilizamos el método del codo para decidir con cuántos factores nos quedamos.
...
La primera variable habla de la popularidad y la segunda del nivel de calidad. Con la primera, vemos que todas las variables relevantes son el número de reseñas y valoraciones, tanto para el autor como para la propia obra. Con la segunda, vemos que las dos variables más importantes hacen referencia a la puntuación media, tanto del autor como de la obra.
...
Vemos que las dos últimas variables, "pages" y "publish_year", tienen una comunalidad prácticamente nula. El resto de variables tienen la comunalidad bastante alta, siendo las que más tienen "author_average_rating" y "book_average_rating".
...
El modelo está bien ajustado, aunque dos variables se quedan bastante fuera. Recordemos que debemos tener un equilibrio entre tener poca unicidad y pocos factores. Lo ideal sería ver qué pasa si añadimos un tercer factor, pero eso es mejor que lo probemos cuando usemos la función "principal".
...
En este diagrama de barras podemos ver las seis variables con alta comunialidad y las dos variables con baja comunalidad.

Hallamos la estimación de la matriz de correlaciones para posteriormente calcular los residuos.
...
Vemos que hay dos variables que se quedan en tierra de nadie, tanto "publish_year" como "pages". También vemos que las variables que comentábamos anteriormente son las que determinan cada uno de los factores, dándonos información sobre popularidad (F1) y sobre nivel de calidad (F2).
...
Comprobamos los valores rotados.
...
Vemos que estas las variables relativas al año de publicación y número de páginas vuelven a tener poca comunalidad. De nuevo, vamos a aumentar el número de factores en pos de que mejoren.
...
Podríamos seguir aumentando el número de factores, y con ello mejorarían las comunalidades; pero esto conllevaría un desequilibrío, ya que nuestro objetivo es minimizar el número de estos.
...
Como podemos comprobar, con 4 factores ninguna variable tiene la comunalidad baja. No obstante, decidimos quedarnos con 3 para tratar de encontrar un equilibrio. Como hemos decidido quedarnos con tres factores, vamos a mostrar de nuevo para estos los valores de cada variable:

```{r}
mt_af_3$loadings
```

Vemos que el primer factor hace referencia de nuevo a la popularidad, el segundo a el nivel o aprobación media y el tercero a libros con pocas páginas y recientes, que puede tener sentido para encontrar aquellos libros que son más o menos modernos y que no tienen una gran cantidad de páginas.

Ahora, comprobemos la complejidad de Hoffman para cada variable, de modo que buscamos que cada una tenga un número cercano a cero.
...
Como vemos, tenemos una complejidad media de 1, lo que es casi perfecto. En general, vemos que casi todas las variables tienen una complejidad cercana a 1, siendo las más altas "pages" y "author_average_rating".

Vemos cómo aportan cada una de las variables a cada factor con la siguiente función.
...
Por último, vamos a mostrar las comunalidades pero aplicando la rotación varimax.
...
Vemos que prácticamente no varía al aplicar esta rotación. Mostramos el digrama de mayor carga para cada variable.
...
Como podemos comprobar, efectivamente no sería un buen ajuste, ya que obtenemos residuos muy altos.

## 4. Análisis de Conglomerados (AC)

En este apartado realizaremos un Análisis de Conglomerados, y para esto aplicaremos el método de las K-medias.
...
Ejecutamos la identificación e imputación de los valors nulos.
...
Primero, al igual que vimos en la práctica informática, vamos a aplicar el método iterativo para comprobar cómo se van recalculando los centroides y con ello reagrupando los puntos.
...
Vemos que los dos centroides han caído entre la "masa" donde se aglomeran casi todos los puntos.
...
Vemos que no se cumple, por lo que seguimos iterando el proceso. A partir de ahora solo mostraremos el gráfico, la varianza total y si se cumple o no la condición.
...
Vemos que los grupos se han generado divididos por los valores de PC1, es decir, los libros con más valoraciones o más populares de los que no lo son. Si recordamos, en nuestro otro caso se dividían por los valores de PC2, dividiendo grupos de libros con valoraciones más altas, frente a los que tienen valoraciones más bajas.

## 5. Conclusiones

Trabajo terminado, como último apartado vamos, de una manera muy breve, a concluir los resultados conseguidos.

Primero, no solo hemos conseguido familiarizarnos mejor con el entorno de RStudio, crenando proyectos y usando diferentes librerías, sino que hemos podido mejorar y desarrollar nuestras habilidades manejando R.

Segundo, hemos tratado unos datos con los que no habíamos trabajado previamente. Con ello, hemos aprendido a manejarnos con datos desconocidos y con un tema que nos parece interesante: el mundo de la literatura.

Por último, hemos podido aplicar todos los conceptos vistos en la asignatura. Hemos repasado su contenido teórico y los hemos aplicado a nuestros datos, de manera que hemos logrado conclusiones interesantes.
